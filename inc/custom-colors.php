...
class GT_Drive_Custom_Colors {
	...
	static function get_custom_colors_css() {
		...
		if ( '' !== $background_color && 'ffffff' !== $background_color ) {
			$color_variables .= '--gt-drive--body-background-color: #' . $background_color . ';';

			// Set Text Color if dark background color was chosen.
			if ( self::is_color_dark( get_theme_mod( 'background_color' ) ) ) {
				$color_variables .= '--gt-drive--text-color: #ffffff;';
				$color_variables .= '--gt-drive--light-text-color: rgba(255, 255, 255, 0.5);';
				$color_variables .= '--gt-drive--light-background-color: rgba(255, 255, 255, 0.05);';
				$color_variables .= '--gt-drive--medium-background-color: rgba(255, 255, 255, 0.15);';
				$color_variables .= '--gt-drive--post-meta-color: rgba(255, 255, 255, 0.5);';
				$color_variables .= '--gt-drive--light-border-color: rgba(255, 255, 255, 0.1);';
				$color_variables .= '--gt-drive--medium-border-color: rgba(255, 255, 255, 0.3);';
				$color_variables .= '--gt-drive--widget-border-color: rgba(255, 255, 255, 0.1);';
				$color_variables .= '--gt-drive--comments-border-color: rgba(255, 255, 255, 0.1);';
			}
		}

		// Set Primary Color.
		if ( $theme_options['primary_color'] !== $default['primary_color'] ) {
			$color_variables .= '--gt-drive--primary-color: ' . $theme_options['primary_color'] . ';';
		}

		// Set Secondary Color.
		if ( $theme_options['secondary_color'] !== $default['secondary_color'] ) {
			$color_variables .= '--gt-drive--secondary-color: ' . $theme_options['secondary_color'] . ';';
		}

		// Set Accent Color.
		if ( $theme_options['accent_color'] !== $default['accent_color'] ) {
			$color_variables .= '--gt-drive--accent-color: ' . $theme_options['accent_color'] . ';';
		}

		// Set Highlight Color.
		if ( $theme_options['highlight_color'] !== $default['highlight_color'] ) {
			$color_variables .= '--gt-drive--highlight-color: ' . $theme_options['highlight_color'] . ';';
		}

		// Set Light Gray Color.
		if ( $theme_options['light_gray_color'] !== $default['light_gray_color'] ) {
			$color_variables .= '--gt-drive--light-gray-color: ' . $theme_options['light_gray_color'] . ';';
		}

		// Set Gray Color.
		if ( $theme_options['gray_color'] !== $default['gray_color'] ) {
			$color_variables .= '--gt-drive--gray-color: ' . $theme_options['gray_color'] . ';';
		}

		// Set Dark Gray Color.
		if ( $theme_options['dark_gray_color'] !== $default['dark_gray_color'] ) {
			$color_variables .= '--gt-drive--dark-gray-color: ' . $theme_options['dark_gray_color'] . ';';
		}

		// Set Header Bar Color.
		if ( $theme_options['header_bar_color'] !== $default['header_bar_color'] ) {
			$color_variables .= '--gt-drive--header-bar-color: ' . $theme_options['header_bar_color'] . ';';

			// Check if a dark background color was chosen.
			if ( self::is_color_dark( $theme_options['header_bar_color'] ) ) {
				$color_variables .= '--gt-drive--header-bar-text-color :#ffffff;';
				$color_variables .= '--gt-drive--header-bar-hover-color: rgba(255, 255, 255, 0.5);';
			}
		}

		// Set Header Bar Color.
		if ( $theme_options['header_bar_icon_color'] !== $default['header_bar_icon_color'] ) {
			$color_variables .= '--gt-drive--header-bar-icon-color: ' . $theme_options['header_bar_icon_color'] . ';';
		}

		// Set Header Color.
		if ( $theme_options['header_color'] !== $default['header_color'] ) {
			$color_variables .= '--gt-drive--header-color: ' . $theme_options['header_color'] . ';';

			// Check if a light background color was chosen.
			if ( self::is_color_light( $theme_options['header_color'] ) ) {
				$color_variables .= '--gt-drive--header-text-color: rgba(0, 0, 0, 0.95);';
				$color_variables .= '--gt-drive--header-hover-color: rgba(0, 0, 0, 0.5);';
			}
		}

		// Set Title Color.
		if ( $theme_options['title_color'] !== $default['title_color'] ) {
			$color_variables .= '--gt-drive--title-color: ' . $theme_options['title_color'] . ';';
			$color_variables .= '--gt-drive--widget-title-color: ' . $theme_options['title_color'] . ';';
		}

		// Set Title Hover Color.
		if ( $theme_options['title_hover_color'] !== $default['title_hover_color'] ) {
			$color_variables .= '--gt-drive--title-hover-color: ' . $theme_options['title_hover_color'] . ';';
			$color_variables .= '--gt-drive--widget-title-hover-color: ' . $theme_options['title_hover_color'] . ';';
		}

		// Set Title Border Color.
		if ( $theme_options['title_border_color'] !== $default['title_border_color'] ) {
			$color_variables .= '--gt-drive--title-border-color: ' . $theme_options['title_border_color'] . ';';
			$color_variables .= '--gt-drive--widget-title-border-color: ' . $theme_options['title_border_color'] . ';';
		}

		// Set Link Color.
		if ( $theme_options['link_color'] !== $default['link_color'] ) {
			$color_variables .= '--gt-drive--link-color: ' . $theme_options['link_color'] . ';';
		}

		// Set Link Hover Color.
		if ( $theme_options['link_hover_color'] !== $default['link_hover_color'] ) {
			$color_variables .= '--gt-drive--link-hover-color: ' . $theme_options['link_hover_color'] . ';';
		}

		// Set Button Color.
		if ( $theme_options['button_color'] !== $default['button_color'] ) {
			$color_variables .= '--gt-drive--button-color: ' . $theme_options['button_color'] . ';';

			// Check if a light background color was chosen.
			if ( self::is_color_light( $theme_options['button_color'] ) ) {
				$color_variables .= '--gt-drive--button-text-color: rgba(0, 0, 0, 0.95);';
			}
		}

		// Set Button Hover Color.
		if ( $theme_options['button_hover_color'] !== $default['button_hover_color'] ) {
			$color_variables .= '--gt-drive--button-hover-color: ' . $theme_options['button_hover_color'] . ';';

			// Check if a light background color was chosen.
			if ( self::is_color_light( $theme_options['button_hover_color'] ) ) {
				$color_variables .= '--gt-drive--button-text-hover-color: rgba(0, 0, 0, 0.95);';
			}
		}

		// Set Footer Color.
		if ( $theme_options['footer_color'] !== $default['footer_color'] ) {
			$color_variables .= '--gt-drive--footer-background-color: ' . $theme_options['footer_color'] . ';';

			// Check if a light background color was chosen.
			if ( self::is_color_light( $theme_options['footer_color'] ) ) {
				$color_variables .= '--gt-drive--footer-text-color: rgba(0, 0, 0, 0.5);';
				$color_variables .= '--gt-drive--footer-link-color: rgba(0, 0, 0, 0.95);';
				$color_variables .= '--gt-drive--footer-link-hover-color: rgba(0, 0, 0, 0.5);';
				$color_variables .= '--gt-drive--footer-border-color: rgba(0, 0, 0, 0.1);';
			}
		}
